...
describe("Web Push subscription (draft/webpush)", function () {
	...
	describe("WEBPUSH LIST (registered devices)", function () {
		const EP2 = "https://push.example.com/send/OtherDevice-987";

		it("accumulates LIST rows and flushes them on the terminator", function () {
			const h = setup();
			register(h, `${ALL_CAPS} draft/webpush=vapid=${VAPID}`);

			h.transport.line(`:irc.test WEBPUSH LIST 1788504150 1 ${ENDPOINT}`);
			h.transport.line(`:irc.test WEBPUSH LIST 1788400000 0 ${EP2}`);

			// Nothing dispatched until the terminator arrives.
			expect(h.payloads("webpush:subscriptions")).to.have.lengthOf(0);

			h.transport.line(":irc.test WEBPUSH LIST * 2");

			expect(h.payloads("webpush:subscriptions")).to.deep.equal([
				{
					network: h.client.uuid,
					subs: [
						{endpoint: ENDPOINT, armed: 1788504150, current: true},
						{endpoint: EP2, armed: 1788400000, current: false},
					],
				},
			]);
		});

		it("dispatches an empty set when the account has no devices", function () {
			const h = setup();
			register(h, `${ALL_CAPS} draft/webpush=vapid=${VAPID}`);

			h.transport.line(":irc.test WEBPUSH LIST * 0");

			expect(h.payloads("webpush:subscriptions")).to.deep.equal([
				{network: h.client.uuid, subs: []},
			]);
		});

		it("starts a fresh set on each WEBPUSH LIST request", function () {
			const h = setup();
			register(h, `${ALL_CAPS} draft/webpush=vapid=${VAPID}`);

			h.transport.line(`:irc.test WEBPUSH LIST 1 1 ${ENDPOINT}`);
			h.client.webpushList(); // a new request resets the accumulator
			h.transport.line(`:irc.test WEBPUSH LIST 2 1 ${EP2}`);
			h.transport.line(":irc.test WEBPUSH LIST * 1");

			const subs = h.payloads("webpush:subscriptions")[0].subs;
			expect(subs.map((s: {endpoint: string}) => s.endpoint)).to.deep.equal([EP2]);
		});
	});
});
