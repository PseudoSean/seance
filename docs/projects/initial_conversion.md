...
12. [x] **File uploads** — point at a network-provided uploader endpoint (configurable per deploy) or drop. _Done 2026-08-25: `branding.uploads.endpoint` (https, multipart POST, JSON `{url}` or text); off by default; contract in `docs/resources/branding.md`. 13 tests._ _Extended 2026-08-28: named presets (`uploads.preset`) plus dotted response paths, extra form `fields`, `optionalFields` retry and an `accept` allowlist, so drag/drop and clipboard paste reach a third-party host. `catbox-litterbox` works today; `boxlabs-paste` awaits a CORS header from its operator. Survey of what else was tested, and the `soju.im/FILEHOST` alternative, in `boxlabs-paste-uploads.md`._
