...
- [ ] Lazy loading: `ensureLanguage` awaits a dynamic import of `prismjs/components/prism-<id>` with a `webpackChunkName: "prism-[request]"` magic comment, after loading the dependencies listed in `components.json` (`require` first, then `optional`/`modify`, following Prism's own loader semantics). Memoise promises per id. Unknown ids resolve `false` without importing. `guessLanguage` dynamically imports `flourite` (`webpackChunkName: "flourite"`).
