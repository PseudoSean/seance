...
| `strings.<key>`                        | string                  | catalog copy (en)                   | Any key in the English catalog (`client/locales/en.json`, compiled from `client/locales/messages.pot`); `normalizeStrings()` in `client/js/branding.ts` accepts every key the catalog carries and silently drops unknown ones. An override is the deploy's voice — in the deploy's language — and wins in every locale, translation included; there is one override per key, and localizing a deploy's own copy per language is out of scope — translations belong in `client/locales/<tag>.po` (`docs/resources/i18n.md`). |
...
| `translation`                          | object                  | none (shipped defaults)             | Client-side translation; see [Translation](#translation).                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `boxlabs-paste`    | The anonymous media staging endpoint of [PASTE](https://github.com/boxlabss/PASTE), `https://paste.boxlabs.uk/img/` — the one poxchat uploads pasted images to. No API key. Images (PNG/JPEG/GIF/WebP) to 10 MB **and video** (MP4/MOV/WebM/AVI/MKV) to 25 MB, kept indefinitely, EXIF stripped server-side. | **Yes** — sends `Access-Control-Allow-Origin: *` _and_ answers `OPTIONS` with `204`, so the strip shows a real percentage                    |

#### Choosing between the two

Both work; they trade different things, and a deploy should pick on purpose rather than inherit the default.

|                | `boxlabs-paste` (stock default)       | `catbox-litterbox`                                   |
| -------------- | ------------------------------------- | ---------------------------------------------------- |
| Images         | PNG/JPEG/GIF/WebP, to 10 MB           | anything, to 1 GB                                    |
| Video          | MP4/MOV/WebM/AVI/MKV, to 25 MB        | anything, to 1 GB                                    |
| Audio, other   | refused                               | accepted                                             |
| Retention      | kept indefinitely                     | **deleted after 1h–72h**                             |
| EXIF           | stripped server-side (`strip_exif=1`) | not touched — the client setting is the only defence |
| Progress strip | real percentage (answers `OPTIONS`)   | indeterminate bar (`405`)                            |

Pick `boxlabs-paste` when a link posted in a channel should still resolve next month — scrollback, a screenshot referenced in a bug report. Pick `catbox-litterbox` for large files, for anything that is not an image or a video, or when uploads _should_ evaporate. Either is one line:
...
`boxlabs-paste` expands to `images[]` as the file field, `strip_exif=1` as an extra field (dropped and retried once if the server says stripping is what failed), `results.0.filePath` / `results.0.error` as the response paths, a 25 MiB limit and PNG/JPEG/GIF/WebP plus MP4/MOV/WebM/AVI/MKV as the accepted types.

The `/img/` path undersells it: the service takes video too, storing it as `vid_*` rather than `img_*`. Two things to know about how it decides. It gates on the **file extension**, not on the type the browser declares — a PNG sent as `video/mp4` is stored as an image, while `.m4v`, `.avif`, audio and a name with no extension at all are refused as `Unsupported type .x` (probed 2026-09-10) — so `accept` here is our MIME-shaped approximation of an extension list, and the two can disagree at the edges; the service's own message is shown verbatim when they do. And its limits differ by kind, 10 MB for images against 25 MB for video, where `uploads.maxSizeBytes` is a single number: the preset carries the **video** figure, so an oversized image is refused by the service rather than by the client. A deploy that shares mostly screenshots can set `maxSizeBytes` to 10 MiB and get the local refusal back.

The endpoint blocked this preset until 2026-09-10: its `POST` response carried no `Access-Control-Allow-Origin`, so the file landed on the server and the browser threw the answer away. The operator added it (`*`, plus a `204` on `OPTIONS`) and an end-to-end upload was verified the same day. **Self-hosted PASTE instances that add their own `/img/` need the same header in their nginx or Apache config** — `/img/` is not in upstream [boxlabss/PASTE](https://github.com/boxlabss/PASTE), so each deployment carries its own. An API key is not a substitute: `api.php` on the same host sends CORS headers, but it only handles text pastes, and CORS is orthogonal to authentication.

Because that service strips EXIF itself (`strip_exif=1`), the "Attempt to remove metadata from images before uploading" setting (which re-encodes through a canvas) is belt-and-braces with `boxlabs-paste` rather than the only defence. With `catbox-litterbox` nothing strips metadata server-side, so the setting is the only thing removing EXIF from a pasted photo. The two differ in the other direction too, and a deploy should pick deliberately: boxlabs keeps what it stores, capped at 10 MB per image and 25 MB per video; litterbox takes a gigabyte of anything but deletes everything within 72 h — good for privacy, bad for scrollback. The re-encode keeps a single frame, so it skips GIF and SVG and — after reading the container header (`client/js/helpers/animatedImage.ts`: the WebP `VP8X` animation flag, a PNG `acTL` chunk, an AVIF `avis` brand) — any animated WebP, APNG or AVIF sequence, which go up byte for byte with their metadata; the confirmation dialog says which it will be. A browser that cannot encode the source format (Safari for WebP, all of them for AVIF) answers with PNG, and the uploaded file then carries that type and a `.png` extension rather than claiming to be what it no longer is.

## Translation

Seance can translate messages on the user's device (`client/js/translate/`, `docs/resources/translation.md`): a WebGPU language model for the languages it handles well and CPU translation models for the rest, chosen per language pair by a route table. Nothing is sent anywhere; the weights are downloaded once into the browser's cache. Everything below is optional.

| Key                     | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `translation.enabled`   | `false` hides the feature. Default `true`.                                                                                                                                                                                                                                                                                                                                                                             |
| `translation.modelBase` | Base URL of a mirror of the weights, absolute or relative to the page (`"models/"`). Unset means the public Hugging Face CDN (`huggingface.co`), which some countries and corporate networks block. Setting it moves the GPU library too (see below).                                                                                                                                                                  |
| `translation.llm`       | `{model, lib}`: a [WebLLM prebuilt model id](https://github.com/mlc-ai/web-llm) (default `Qwen3-1.7B-q4f16_1-MLC`) and, for a model that is not prebuilt, the URL of its compiled `.wasm` library. Users choose the GPU model in Settings between `Qwen3-1.7B-q4f16_1-MLC` and `Qwen3-4B-q4f16_1-MLC`; `model` becomes the default choice and, when it is neither, is added as a third. `lib` applies to `model` only. |
| `translation.cpu`       | `{nllb, opus}`: the NLLB repo (default `Xenova/nllb-200-distilled-600M`) and OPUS-MT repos keyed `"from-to"` (e.g. `{"fi-en": "Xenova/opus-mt-fi-en"}`), added to the shipped pairs.                                                                                                                                                                                                                                   |
| `translation.routes`    | Route overrides: `{"<to>": {"<from>" or "*": ["llm", "opus:de-en", "nllb"]}}`, merged over the shipped table one entry at a time. The list is quality classes, best first: a string is a class of one (so a flat list is a strict order), a nested list a class of equivalent candidates, the downloaded one preferred (`[["llm", "opus:de-en"], "nllb"]`).                                                            |
| `translation.glossary`  | `[["rig", "Testaufbau"], …]`: network vocabulary seeded into every channel's term memory.                                                                                                                                                                                                                                                                                                                              |

**Hosts a default deploy contacts.** With no `translation` block, the first translation on a device downloads from two third-party hosts: **`huggingface.co`** (the weights, both tiers — `cdn-lfs*.hf.co` for the large files) and **`raw.githubusercontent.com`** (the compiled WebLLM GPU library, `mlc-ai/binary-mlc-llm-libs`). Nothing else leaves the device — no text is ever sent anywhere — but a deploy that must keep its users off those hosts mirrors them.

**Mirror layout.** Under `modelBase`, a WebLLM model lives at `<modelBase>/<model_id>/` with the files of its Hugging Face repo (`mlc-chat-config.json`, `ndarray-cache.json`, the `params_shard_*.bin`), and a transformers.js model at `<modelBase>/<repo>/` (e.g. `<modelBase>/Xenova/nllb-200-distilled-600M/`) with `config.json`, `tokenizer.json`, `tokenizer_config.json` and `onnx/*_quantized.onnx`. A mirror on another origin needs `Access-Control-Allow-Origin` for the app's origin; a mirror inside the app's own tree goes under `models/`, which the service worker leaves alone — and a relative `modelBase` (`"models/"`, `"../weights/"`) is resolved against the page, so the deploy does not have to name its own origin.

**A mirror is a whole mirror.** Setting `modelBase` also moves the GPU library: it is fetched from `<modelBase>/<the library's file name>` (e.g. `models/Qwen3-1.7B-q4f16_1-ctx4k_cs1k-webgpu.wasm`), not from `raw.githubusercontent.com`, so copy the prebuilt model's `model_lib` file next to the weights. `translation.llm.lib` names the library outright and wins over that rule — use it for a model that is not prebuilt, or for a library served from somewhere else. Everything else the prebuilt record says about the model (its context-window overrides, its VRAM requirement) is kept as it is.
