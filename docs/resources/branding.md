...
Capping `maxSizeBytes` below the service's own limit is still worth considering even though the progress strip shows real byte counts (the request goes over `XMLHttpRequest` for its upload progress events): a gigabyte over a phone connection is a long wait either way. Overriding `endpoint` is how a deploy aims a preset's wire format at its own instance.
