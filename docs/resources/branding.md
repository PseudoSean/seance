...
| `strings.<key>`                        | string                  | catalog copy (en)                   | Keys: `connect.title`, `connect.savedNetworks`, `connect.savedNetworksEmpty`, `connect.submit`, `connect.signInTitle`, `connect.signInIntro`, `connect.signInSubmit`, `connect.rememberMe`, `connect.guestTitle`, `connect.guestSubmit`, `help.about`, `help.website`, `help.documentation`, `help.privacy`. An override is the deploy's voice — in the deploy's language — and wins in every locale, translation included; there is one override per key, and localizing a deploy's own copy per language is out of scope. |
