...
import {readAvailableLocales} from "./tools/i18n/available-locales";
...
// substituted wherever `__AVAILABLE_LOCALES__` appears in index.html. The
// generated file depends on no build mode (tools/i18n/compile.ts), so the
// dev-only rig is dropped for a production build by the reader itself —
// the inline script is baked at build time and can check nothing at
// runtime, unlike the selector and activate(), which fold on NODE_ENV
// themselves. What it does is pinned by test/tests/i18n-toolchain.ts.
const availableLocales = readAvailableLocales(
	path.resolve(__dirname, "client/locales/tags.json"),
