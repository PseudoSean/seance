...
// The locales this build ships, for the pre-paint language resolution
// (Task 3's inline script): `client/locales/tags.json` is generated by
// `yarn i18n:compile`, which `yarn build` runs before webpack, and is
// substituted wherever `__AVAILABLE_LOCALES__` appears in index.html.
// The generated file depends on no build mode (tools/i18n/compile.ts), so
// the dev-only rig is dropped here instead — the inline script is baked at
// build time and can check nothing at runtime, unlike the selector and
// activate(), which fold on NODE_ENV themselves.
// Spelled out rather than imported (tools/ is a separate TS project):
// test/tests/i18n-toolchain.ts pins this list to tools/i18n/paths.ts's
// DEV_ONLY_TAGS, as the RTL set is pinned between core.ts and index.html.
const DEV_ONLY_TAGS = ["qqx"];
...
function readAvailableLocales(): string[] {
	...
	try {
		raw = readFileSync(path.resolve(__dirname, "client/locales/tags.json"), "utf8");
	} catch (e: any) {
		throw new Error(
			`client/locales/tags.json is missing or unreadable — run "yarn i18n:compile" first: ${e.message}`
		);
	}
	...
	if (!isTagList(tags)) {
		throw new Error("client/locales/tags.json must be a JSON array of language tags");
	}

	return process.env.NODE_ENV === "production"
		? tags.filter((tag) => !DEV_ONLY_TAGS.includes(tag))
		: tags;
}

const availableLocales = readAvailableLocales().join(",");
...
function brandHtml(content: string): string {
	return (
		content
			...
			// Inside the pre-paint script's quoted string: a bare comma-joined
			// tag list, not escaped — the tags come from our own compile run.
			.replace(/__AVAILABLE_LOCALES__/g, availableLocales)
	);
}
		...
		new CopyPlugin({
			patterns: [
				...
				},
				{
					...
					// notifications: the worker composes its copy through the
					// same i18n runtime (client/js/push/i18n.ts), but a classic
					// service worker may only importScripts while it installs,
					// so the overlay is fetched same-origin from
					// <app>/locales/<tag>.json at push time instead of
					// webpack-imported. en.json also ships inside js/push.js
					// (core.ts imports it statically); these copies serve every
					// other tag. Regenerated by `yarn build`'s i18n:compile.
					// The qqx rig never ships: production copies omit it (the
					// runtime cannot select it there anyway).
					from: path.resolve(__dirname, "./client/locales/*.json"),
					to: "locales/[name][ext]",
					globOptions: {
