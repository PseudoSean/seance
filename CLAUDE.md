...
- **Web push** — `client/js/webpush.ts` (Vue + store, so not under mocha) owns the one browser `PushSubscription` (`thelounge.push`, keyed by the server's VAPID key), re-REGISTERs it on every `webpush:available`, and decides the connect-time prompt (`PushPrompt.vue`, yes/no/never): `subscribe` when nothing is stored (skipped when permission is already granted — it just subscribes; `thelounge.push.neverAsk`), `renew` when the stored subscription matches no key a connected server announces (a VAPID rotation) — governed by the `pushKeyChange` setting (Wary `ask` prompt / Naive `trust` renew silently / Suspicious `ignore`; the prompt's Never sets `ignore`). **A rotation is never healed silently**: the browser refuses `subscribe()` with a new key until the old subscription is dropped (`pushSubscription()` does that, the caller unregisters the old endpoint everywhere), and `refreshState` reports `stale`: Settings → Notifications says so, and the Edit-network form offers Renew for that network (`networkPushInfo(uuid).stale`, `webpush.renew(uuid)`). The key/stale logic is Vue-free in `helpers/pushKeys.ts` (`test/helpers/pushKeys.ts`); the browser flow is `tools/scenarios/push-renew.mjs` (faked Push API via `page.addInitScript`). See `docs/projects/push-subscription.md` § VAPID rotation.
