...
- **Reconnects are quiet.** A channel we were in when the connection dropped has `Channel.rejoining` set until its re-JOIN's 366; a `draft/persistence` restoration batch runs with `client.restoring` set. `join.ts` hides our own JOIN and `topic.ts` an unchanged 332/333 while either is set — the catch-up (`CHATHISTORY AFTER`) is what shows what happened. Keep new join-burst handlers consistent with this.
