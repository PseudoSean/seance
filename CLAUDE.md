...
- **Markdown** — what a message renders as is decided by `layout()` (`client/js/helpers/ircmessageparser/layout.ts`), a Vue-free `LayoutNode` tree: `parseStyle → applyMarkdown → finders → merge → layout tree`, then an adapter — `parse.ts` (`toVNodes`, the Vue one) or `toPlainText` (`Chat.vue`'s `plainTopic`). Markdown itself is `parseMarkdown.ts` (`tokenize` → `Piece[]`, `applyMarkdown` → `{fragments, verbatim}`); neither file imports Vue, the store or the DOM, so mocha loads them: `test/helpers/layout.ts` and `test/helpers/parseMarkdown.ts`. Vocabulary in `CONTEXT.md`; see `docs/projects/markdown-messages.md`.
